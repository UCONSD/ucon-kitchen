...
## Вывод терминала: команда `cl`

Терминал Андрея агенту не виден — оболочка моста это отдельная песочница, а не его Мак.
Не предлагай перенаправлять вывод во временные файлы, канал уже есть.

Команды для Андрея давай с префиксом `cl`:

```
cl 'cd ~/dev/ucon-kitchen && npm test'
```

Программа `~/.ucon/bin/cl` выполняет команду, показывает вывод на экране и пишет
стенограмму в `~/dev/_claude/terminal.log` — машина, учётка, папка, проект, команда,
вывод, код выхода. Этот файл читай сам через мост; папка `~/dev` должна быть
подключена к сессии.

Аргументы с пробелами заворачивай в одинарные кавычки целиком — двойные съедаются
при передаче.

Скриншоты лог не заменяет: визуальное (окна настроек, строка меню, цвета) по-прежнему
приходит от Андрея.
